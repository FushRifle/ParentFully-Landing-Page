...
const PartnerAgreementModal = lazy(() => import('@/components/affiliate/PartnerAgreementModal'));
const SchoolAffiliateTermsModal = lazy(() => import('@/components/affiliate/SchoolAffiliateTermsModal'));

function AgreementLoading() {
    return <div role="status" className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 text-sm font-bold text-white">Loading terms…</div>;
}
...
function getErrorMessage(payload: unknown, status: number, isSchoolApplication = false): string {
...
export default function AffiliateLandingContent({ isSchoolPartnership }: { isSchoolPartnership: boolean }) {
    const [countryOptions, setCountryOptions] = useState<Array<{ label: string; value: string }>>([]);

    useEffect(() => {
        if (isSchoolPartnership) return;
        let active = true;
        import('react-select-country-list').then(({ default: countryList }) => {
            if (active) setCountryOptions(countryList().getData());
        });
        return () => {
            active = false;
        };
    ...
    const submitSchoolInterest = async (event: FormEvent<HTMLFormElement>) => {
        ...
        for (const [value, label] of [
            [schoolForm.classCount, 'number of classes'],
            [schoolForm.familyCount, 'approximate number of families'],
        ]) {
            if (value.trim() && (!/^[1-9]\d*$/.test(value.trim()) || !Number.isSafeInteger(Number(value)))) {
                return setError(`Please enter a whole number greater than zero for ${label}.`);
            }
        }
    ...
    return (
        <div className="min-h-screen bg-[#F6FAF7] pt-24 sm:pt-32">
                ...
                <div className="relative mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
                    ...
                    <div className="relative mx-auto w-full max-w-xl">
                        ...
                        <div className="relative overflow-hidden rounded-[2.25rem] border border-white/20 bg-white/10 p-2 shadow-[0_30px_80px_rgba(0,0,0,0.25)] backdrop-blur">
                            <Image
                                src={isSchoolPartnership ? '/images/school-partnership-hero.webp' : '/images/parenting-team-phone-diverse.png'}
                        ...
                        {submitted ? (
                            ...
                            <SchoolAffiliateApplicationForm
                                ...
                                onAcceptedChange={(accepted) => {
                                    setSchoolTermsAccepted(accepted);
                                    if (error) setError('');
                                }}
            ...
            {agreementOpen && <Suspense fallback={<AgreementLoading />}><PartnerAgreementModal
                open={agreementOpen}
            ...
            /></Suspense>}
            {schoolTermsOpen && <Suspense fallback={<AgreementLoading />}><SchoolAffiliateTermsModal
                open={schoolTermsOpen}
                onClose={() => setSchoolTermsOpen(false)}
            /></Suspense>}
        </div>
    );
}
